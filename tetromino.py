...
class Tetromino:
   ...
   def turnTetromino(self,grid):
      n = len(self.tile_matrix)  # n = number of rows = number of columns
      rotatedMatrix = np.full((n, n), None)
      for row in range(n):
         for col in range(n):
            currentTetromino = self.tile_matrix[row][col]
            if currentTetromino != None:
               ...
               a = (b-(n-1))*-1
               b = temp
               rotatedMatrix[a][b] = currentTetromino
      if self.CheckTetromino(rotatedMatrix,grid):
         self.tile_matrix = rotatedMatrix
   ...
   def reverseTurnTetromino(self,grid):
      n = len(self.tile_matrix)  # n = number of rows = number of columns
      rotatedMatrix = np.full((n, n), None)
      for row in range(n):
         for col in range(n):
   ...
   def CheckTetromino(self,rotatedMatrix,grid):
      n = len(self.tile_matrix)  # n = number of rows = number of columns
      for row in range(n):
         for col in range(n):
            currentTile= rotatedMatrix[row][col]
            if currentTile != None:
               position = self.get_cell_position(row,col)
               if(not grid.is_inside(position.y,position.x) or grid.tile_matrix[position.y][position.x] != None):
                  return False
      return True


   def can_be_moved(self, dir, game_grid):
      n = len(self.tile_matrix[0])  # n = number of rows = number of columns
